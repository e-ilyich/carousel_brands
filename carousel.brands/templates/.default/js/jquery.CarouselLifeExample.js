...
/********************************************************/
(function($){
        $.fn.Carouselil = function(options) {
                // Настройки по умолчанию
                var settings = {
                        visible: 4,  //количество отображаемых позиций 3
                        rotateBy: 1, //прокручивать по 1
                        speed: 1000, //скорость 1 секунда
                        btnNext: null, // кнопка вперед не назначена
                        btnPrev: null, // кнопка назад не назначена
                ...
                return this.each(function() {
                        ...
                        var $carouselil = $this.children(':first');// получаем дочерний элемент (UL) т.е. саму карусель
						var itemWidth = $carouselil.children().outerWidth()+settings.margin; // вычисляем ширину элемента
						var itemHeight = $carouselil.children().outerHeight()+settings.margin;// вычисляем высоту элемента
					    var itemsTotal = $carouselil.children().length; // получаем общее количество элементов в каруселе
	...
	$carouselil.children(':last').after($carouselil.children().slice(0,settings.rotateBy).clone(true));
                                                ...
                                                * в параметре rotateBy (по умолчанию задан один элемент)
                                                */
                                                $carouselil.children(':first').before($carouselil.children().slice(itemsTotal - settings.rotateBy, itemsTotal).clone(true));
                                                ...
                                                */
												if(settings.position=="v")
                                                $carouselil.css('top', -size * settings.rotateBy + 'px');
												else 
												$carouselil.css('left', -size * settings.rotateBy + 'px');
                                        }
                                        ...
                                        */
										if(settings.position=="v")
                                        Indent = parseInt($carouselil.css('top')) + (size * settings.rotateBy * direction);
                                        else
										Indent = parseInt($carouselil.css('left')) + (size * settings.rotateBy * direction);
										...
										// запускаем анимацию
                                        $carouselil.animate(animate_data, {queue: false, duration: settings.speed, complete: function() {
                                                                                   
										  // когда анимация закончена
                                                if (!dir) { // если мы мотаем к следующему элементу (так по умолчанию)
                                                        // удаляем столько первых элементов, сколько задано в rotateBy
                                                        $carouselil.children().slice(0, settings.rotateBy).remove();
                                                        // устанавливаем сдвиг в ноль
														if(settings.position=="v")
                                                        $carouselil.css('top', 0);
														else
														$carouselil.css('left', 0);
                                                } else { // если мотаем к предыдущему элементу
                                                        // удаляем столько последних элементов, сколько задано в rotateBy
                                                        $carouselil.children().slice(itemsTotal, itemsTotal + settings.rotateBy).remove();
                                                }
